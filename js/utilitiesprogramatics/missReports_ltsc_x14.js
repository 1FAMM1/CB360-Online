...
async function loadocrReportsFromSupabase(total = 24) {
  ...
  try {
    ...
    // leitura segura (por causa de respostas vazias/estranhas)
    ...
    data.slice(0, total).forEach((report, index) => {
      ...
      // ✅ IDs CERTOS (iguais aos que crias no createOcrReportsInputs)
      const nIntInput = document.getElementById(`report-${n}-nint`);
      const nrInput   = document.getElementById(`report-${n}-report-nr`);
      const dateInput = document.getElementById(`report-${n}-report-date`);
...
async function saveOcrReportsToSupabase(total = 24) {
  ...
  for (let i = 1; i <= total; i++) {
    ...
    const nrEl   = document.getElementById(`report-${n}-report-nr`);
    const dateEl = document.getElementById(`report-${n}-report-date`);
    ...
    const status      = (stEl?.value || "").trim(); // done | pending | ''
    ...
    rows.push({
      corp_oper_nr,
      n_int,
      report_nr: report_nr || null,
      report_date: report_date || null,
      report_state: status === "done"
    });
  }
  ...
  try {
    // 2) ✅ FETCH CERTO (faltava no teu código)
    ...
    // 3) Notificações só para n_int inseridos
    try {
      const now = new Date().toISOString();
      const uniqueNints = [...new Set(rows.map(r => r.n_int).filter(n => Number.isInteger(n)))];

      if (uniqueNints.length > 0) {
        const msgNotif = `📄 Existem atualizações nos seus relatórios pendentes. Consulte "Relatórios".`;

        const notifications = uniqueNints.map(nint => ({
          n_int: nint,
        ...
        for (const nint of uniqueNints) {
          try {
            await fetch("https://cb-360-app.vercel.app/api/sendPush", {
              ...
              body: JSON.stringify({
                recipient_nint: nint.toString(),
                corp_nr: corp_oper_nr,
                sender_name: "CB360 Online",
