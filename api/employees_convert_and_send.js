...
function makeRowXml(rowNum, emp) {
  ...
  const values = {
    B: emp.name, C: emp.subShift, D: emp.casualties,
    E: emp.vacations, F: emp.parental, G: emp.disgust,
    H: emp.justified, I: emp.unjustified,
  };
...
/**
 * Converts a workbook (ExcelJS) to PDF via Adobe PDF Services.
 * Writes a temp XLSX, uploads it, polls for the result, streams back the PDF
 * buffer, then cleans up both temp files.
 *
 * @param {ExcelJS.Workbook} workbook
 * @param {string} prefix   - short prefix for the temp file names
...
/**
 * Same as workbookToPdfBuffer but accepts a raw Buffer (pre-built XLSX)
 * instead of an ExcelJS workbook. Used by salary-map handlers that build
 * the XML directly with AdmZip.
 *
...
export default async function handler(req, res) {
  ...
  const VALID_MODES = [
    "monthly_scales", "point_sheet", "shift_allowance", "detailed_shift_allowance",
    "vacation_form", "vacation_map", "vacation_anomalies", "vacation_priority",
    "salary_map", "salary_map_xlsx", "eip_annual_map",
  ];
...
/**
 * Downloads the salary-map template, injects the header title and all data
 * rows via direct XML manipulation (AdmZip), and returns the modified XLSX
 * as a Buffer together with the month name.
 */
async function buildSalaryMapXlsx(year, month, employees) {
  ...
  // Re-extract header rows 2-4, 6-8 from the original XML
  const headerRows = [];
  ...
  // Patch the title in row 6 / cell B6
...
async function handleSalaryMap(req, res) {
  ...
  const { zip, beforeSheetData, afterSheetData, headerRows, dataRowsXml, monthName } =
    await buildSalaryMapXlsx(year, month, employees);
...
async function handleSalaryMapXlsx(req, res) {
  ...
  const { zip, beforeSheetData, afterSheetData, headerRows, dataRowsXml } =
    await buildSalaryMapXlsx(year, month, employees);
...
async function handleEIPAnnualMap(req, res) {
  ...
  for (let mi = 0; mi < 12; mi++) {
    const month      = mi + 1;
    const colBase    = 2 + (mi * 3); // day column
    const colWd      = colBase + 1;  // weekday column
    const colTeam    = colBase + 2;  // team column
    const daysInMonth = new Date(year, month, 0).getDate();
