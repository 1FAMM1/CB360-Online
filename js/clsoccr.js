...
function generateCloseCREPCMessage() {
  ...
  document.querySelectorAll('.vehicle-card').forEach(card => {
    ...
    const lines = [];
    if (chTODate || chTOTime) lines.push(`*GDH Ch TO:* ${vehicle} | ${formatGDH(chTODate, chTOTime)}`);
    if (sdTODate || sdTOTime) lines.push(`*GDH Sd TO:* ${vehicle} | ${formatGDH(sdTODate, sdTOTime)}`);
    if (chUndDate || chUndTime || kms) {
      let line = `*GDH Ch Und:* ${vehicle} | ${formatGDH(chUndDate, chUndTime)}`;
      if (kms) line += ` | ${kms} Kms`;
      lines.push(line);
    }
