...
export default async function handler(req, res) {
  ...
  try {
    ...
    // --- LÓGICA DE FERIADOS (Ajustada para suportar cores diferentes) ---
    function getPortugalHolidays(y) {
      ...
      // Feriados normais (Rosa)
      fixed.forEach(f => holidayMap.set(`${y}-${f.m}-${f.d}`, "F7C6C7"));
      [add(easter,-2), easter, add(easter,60)].forEach(dt => holidayMap.set(`${dt.getFullYear()}-${dt.getMonth()+1}-${dt.getDate()}`, "F7C6C7"));
      
      // Carnaval (Azul Claro)
      const carv = add(easter, -47);
      holidayMap.set(`${carv.getFullYear()}-${carv.getMonth()+1}-${carv.getDate()}`, "D6ECFF");
    ...
    for (let d = 1; d <= 31; d++) {
      ...
      if (d <= daysInMonth) {
        ...
        const holidayColor = holidays.get(`${year}-${month}-${d}`);
        ...
        let dateBg = holidayColor || (isWeekend ? WEEKEND_COLOR : null);
        ...
        // 2. PINTAR TURNO (Com regra de Driver/FE)
        const bgHex = normalizeHex6(employee.cellColors?.[d - 1] || "FFFFFF");
        cellTurno.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FF" + bgHex } };
        
        // Regra de texto: Drivers (Rosa Choque) e FE (Azul) sempre PRETO
        const isSpecialCase = (bgHex === "FF69B4" || bgHex === "00B0F0");
        const textColor = isSpecialCase ? "FF000000" : (isDarkHex(bgHex) ? "FFFFFFFF" : "FF000000");
