...
export default async function handler(req, res) {
  ...
  const { to, subject, message, corpOperNr, corpName, logoUrl, senderName, isBulk } = req.body;
  ...
  // CONFIGURAÇÃO DINÂMICA DOS CAMPOS DE DESTINO
  let mailOptions = {
  ...
  // Se for um envio em massa (Bulk), coloca a lista no campo cc
  if (isBulk && Array.isArray(to)) {
    mailOptions.to = process.env.GMAIL_EMAIL; // Envia para a própria corporação no campo principal
    mailOptions.cc = to;                       // Toda a lista de bombeiros fica visível em CC
  } else {
    // Caso contrário, continua a ser o envio individual normal para um só contacto
    mailOptions.to = to;
  }
