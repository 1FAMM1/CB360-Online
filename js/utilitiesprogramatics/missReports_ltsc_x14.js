  ...
  return;
        ...
        try {
          ...
          const uniqueNints = [...new Set(
            rows.map(r => r.n_int).filter(n => Number.isInteger(n))
          )];
          if (uniqueNints.length > 0) {
            const msgNotif = `📄 Existem atualizações nos seus relatórios pendentes. Consulte "Relatórios".`;
            const notifications = uniqueNints.map(nint => ({n_int: nint, corp_oper_nr: corp_oper_nr, title: "Relatórios Atualizados", message: msgNotif, is_read: false, created_at: now}));
