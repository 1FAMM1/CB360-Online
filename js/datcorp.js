/* --- Função para Salvar Dados da Corporação --- */
    async function saveCorpGroupFields() {
      const saveButton = document.querySelector('button[onclick="saveCorpGroupFields()"]');
      ...
      try {
        ...
        const existingRecord = await checkExistingCorporation();
        let response;
        if (existingRecord) {
          response = await updateCorporationData(existingRecord.id, corporationData);
        } else {
    ...
    function splitPostalCode(fullPostalCode) {
      ...
      if (fullPostalCode.length === 7 && /^\d{7}$/.test(fullPostalCode)) {
        return {
          cp1: fullPostalCode.substring(0, 4),
          cp2: fullPostalCode.substring(4, 7)
        };
      }
      return {
        cp1: fullPostalCode,
    ...
    function validateCorporationData(data) {
      if (!data.corporation) {
        return {
          isValid: false,
          message: 'Nome da corporação é obrigatório'
        };
      }
    ...
    async function checkExistingCorporation() {
      try {
        const response = await fetch(`${SUPABASE_URL}/rest/v1/corporation_data?select=id&limit=1`, {
    ...
    async function loadCorporationData() {
      try {
        ...
        // Código postal - separar em dois campos
        const {
          cp1,
          cp2
        } = splitPostalCode(corporation.corp_cp);
      ...
      try {
        if (corporation.corp_district) {
          ...
          if (districtId) {
            ...
            await populateCouncilSelectsByDistrict(districtId, 'district_select_corp');
          }
        }
