    ...
    export default async function handler(req, res) {
      ...
      try {
        ...
        if (type === "contact_list") {
          const quadroMap = {QCOM: {startRow: 9, endRow: 11}, QATIV: {startRow: 17, endRow: 116}, QEST:  {startRow: 122, endRow: 142},
    ...
    if (type === "email") {
      ...
      const htmlTemplate = `
        <div style="font-family:Arial">
          ${logoUrl ? `<img src="${logoUrl}" style="height:80px" />` : ""}
          <h3>${corpName || ""}</h3>
      ...
      if (attachment) {mailOptions.attachments = [{filename: attachment.filename, content: attachment.content, encoding: "base64"}];}
      try {
