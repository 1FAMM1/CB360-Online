    ...
    export default async function handler(req, res) {
      ...
      try {
        ...
        try {
          ...
          newSheet.columns = [{width: 5}, {width: 5}, {width: 8}, {width: 20}, {width: 8}];
          for (let d = 1; d <= data.daysInMonth; d++) {
          ...
          for (let d = 1; d <= data.daysInMonth; d++) {
            const col = 6 + (d - 1);
            newSheet.getCell(11, col).value = data.weekdays[d - 1] || '';
            newSheet.getCell(12, col).value = d;
            if (holidayDays.includes(d)) {
              newSheet.getCell(8, col).value = 'FR';
            }
          }            
          let rowNum = 15;
          data.fixedRows.forEach(fixedRow => {
            ...
            for (let d = 1; d <= data.daysInMonth; d++) {
              const col = 6 + (d - 1);
              newSheet.getCell(rowNum, col).value = fixedRow.days[d] || '';
            }
            rowNum++;
          });
          rowNum = 18;
          data.normalRows.forEach(normalRow => {
            ...
            for (let d = 1; d <= data.daysInMonth; d++) {
              const col = 6 + (d - 1);
              newSheet.getCell(rowNum, col).value = normalRow.days[d] || '';
            }
            rowNum++;
          });
          for (let r = 18; r <= 117; r++) {
