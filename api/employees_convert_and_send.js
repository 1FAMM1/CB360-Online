    ...
    function setFill(cell, hex6) {
      breakStyle(cell);
      cell.fill = {type: "pattern", pattern: "solid", fgColor: {argb: "FF" + normalizeHex6(hex6)}};
    }
    ...
    function setFontKeepTemplate(cell, {bold = null, italic = false, bgHex = "FFFFFF", forceTextColor = null} = {}) {
      ...
      cell.font = {...base, ...(bold === null ? {} : {bold: !!bold}), italic: !!italic, color: {argb: colorArgb},};
    }
    function setBorder(cell) {
      breakStyle(cell);
      const c = {argb: "FFD1D1D1"};
      cell.border = {top: {style: "thin", color: c}, left: {style: "thin", color: c}, bottom: {style: "thin", color: c}, right:  {style: "thin", color: c},};
    }
    ...
    // ─── XML helpers (used by salary-map handlers) ───────────────────────────────
...
function makeRowXml(rowNum, emp) {
  ...
  const styles = isFirst
    ? { B: 10, C: 7, D: 7, E: 7, F: 7, G: 7, H: 7, I: 7, J: 11 }
    : { B: 12, C: 8, D: 8, E: 8, F: 8, G: 8, H: 8, I: 8, J: 13 };
  const cols = ["B", "C", "D", "E", "F", "G", "H", "I", "J"];
  const values = {
    B: emp.name, C: emp.subShiftCurrent, D: emp.subShiftPrevious,
    E: emp.casualties, F: emp.vacations, G: emp.parental,
    H: emp.disgust, I: emp.justified, J: emp.unjustified,
  };
  ...
  return `<row r="${rowNum}" spans="2:9" ht="20" x14ac:dyDescent="0.25">${cells}</row>`;
    ...
    async function xlsxBufferToPdfBuffer(xlsxBuffer, prefix = "doc") {
      const tempDir = os.tmpdir();
      const inputFilePath = path.join(tempDir, `${prefix}_${Date.now()}.xlsx`);
      const outputFilePath = path.join(tempDir, `${prefix}_${Date.now()}_out.pdf`);
      const cleanup = () => {
    ...
    // ─── Shared month / weekday name tables ──────────────────────────────────────
    ...
    export default async function handler(req, res) {
      ...
      const VALID_MODES = ["monthly_scales", "point_sheet", "shift_allowance", "detailed_shift_allowance", "vacation_form", "vacation_map", "vacation_anomalies",
                           "vacation_priority", "salary_map", "salary_map_xlsx", "eip_annual_map",];
      try {
        const { mode } = req.body;
        ...
        if (mode === "shift_allowance") return await handleShiftAllowance(req, res);
        if (mode === "detailed_shift_allowance") return await handleDetailedShiftAllowance(req, res);
    ...
    async function handleMonthlyScales(req, res) {
      ...
      for (let d = 1; d <= 31; d++) {
        worksheet.getColumn(DAY_START_COL + (d - 1)).hidden = d > daysInMonth;
      }
      const GROUP_RANGES = {INEM: {start: 13, end: 32}, TDNU: {start: 34, end: 39}, OPC:  {start: 41, end: 45}, EP1: {start: 47, end: 51}, EP2: {start: 53, end: 57},};
      ...
      function fillEmployeeAtRow(emp, excelRow) {
        ...
        for (let d = 0; d < daysInMonth; d++) {
          ...
          breakStyle(cell);
          const shift = (emp.shifts?.[d] || "").toString();
          cell.value = shift;
      ...
      ["INEM", "TDNU", "OPC", "EP1", "EP2"].forEach((key) => {
      ...
      const pdfBuffer = await workbookToPdfBuffer(workbook, `Escala_${month}_${year}`);
    ...
    async function handlePointSheet(req, res) {
      ...
      for (let d = 1; d <= 31; d++) {
        const row = 11 + d;
        const cellDay = worksheet.getCell(row, 2);
        const cellWeek = worksheet.getCell(row, 3);
        const cellShift = worksheet.getCell(row, 4);
        breakStyle(cellDay);
        breakStyle(cellWeek);
        breakStyle(cellShift);
        if (d <= daysInMonth) {
          const date = new Date(year, month - 1, d, 12, 0, 0);
          ...
          cellDay.value = d;
          cellWeek.value = WEEKDAY_NAMES[wDay];
          cellShift.value = employee.shifts?.[d - 1] || "";
          let dateBg = null;
          if (holiday) dateBg = holiday.optional ? HOLIDAY_OPTIONAL_COLOR : HOLIDAY_COLOR;
          else if (isWeekend) dateBg = WEEKEND_COLOR;
          if (dateBg) {
            setFill(cellDay, dateBg);
            setFill(cellWeek, dateBg);
            setFontKeepTemplate(cellDay, {bold: true, italic: false, bgHex: dateBg});
            setFontKeepTemplate(cellWeek, {bold: true, italic: false, bgHex: dateBg});
          }
          const bgHex = normalizeHex6(employee.cellColors?.[d - 1] || "FFFFFF");
          setFill(cellShift, bgHex);
          setFontKeepTemplate(cellShift, {bold: true, italic: false, bgHex: bgHex});
          [cellDay, cellWeek, cellShift].forEach((c) => {
      ...
      const pdfBuffer = await workbookToPdfBuffer(workbook, "point_sheet");
      res.setHeader("Content-Type", "application/pdf");
      return res.status(200).send(pdfBuffer);
    }
    async function handleShiftAllowance(req, res) {
      ...
      employees.forEach((emp, i) => {
        ...
        COLS.forEach((col, idx) => {ws.getCell(`${col}${r}`).value = emp.months[idx] || ""; });
      });
      for (let r = ROW_START + employees.length; r <= ROW_MAX; r++) ws.getRow(r).hidden = true;
      ws.pageSetup = {orientation: "landscape", paperSize: 9, fitToPage: true, fitToWidth: 1, fitToHeight: 0, horizontalCentered: true,
                      margins: {left: 0.25, right: 0.25, top: 0.75, bottom: 0.25, header: 0, footer: 0},};
      const pdfBuffer = await workbookToPdfBuffer(ws._workbook ?? workbook, "shift_allowance");
      res.setHeader("Content-Type", "application/pdf");
    ...
    async function handleDetailedShiftAllowance(req, res) {
      ...
      const monthLabel = (!month || month === "all" || isNaN(monthNum)) ? "Todos os Meses" : MONTH_NAMES_FULL[monthNum];
      ...
      records.forEach((rec, i) => {
        ...
        ws.getCell(`D${r}`).value = `${String(rec.day).padStart(2, "0")} ${MONTH_NAMES_FULL[recMonth]} ${year}`;
        ws.getCell(`E${r}`).value = rec.exit_hour || "--:--";
        ws.getCell(`F${r}`).value = "ELEGÍVEL";
      });
      for (let r = ROW_START + records.length; r <= ROW_MAX; r++) ws.getRow(r).hidden = true;
      ...
      const pdfBuffer = await workbookToPdfBuffer(workbook, "shift_detail");
      res.setHeader("Content-Type", "application/pdf");
    ...
    async function handleVacation(req, res) {
      ...
      await workbook.xlsx.load(await templateResponse.arrayBuffer());
      const worksheet = workbook.worksheets[0];
      function padNumber(num, length) {return String(num).padStart(length, "0");}
    ...
    async function handleVacationMap(req, res) {
      ...
      employees.forEach((emp, index) => {
        ...
        for (let m = 1; m <= 12; m++) {
          ...
          const startPeriods = periods.filter((p) => p.sM === m);
          if (startPeriods.length === 0) continue;
          ...
          const span = (maxEM - m) + 1;
          const sCol = 2 + m;
          const txt  = allInBlock
          ...
          .join(" e ");
          if (span > 1) {
            try {worksheet.mergeCells(row, sCol, row, sCol + (span - 1));} catch {}
          }
      ...
      worksheet.pageSetup = {orientation: "landscape", paperSize: 9, fitToPage: true, fitToWidth: 1, fitToHeight: 0, horizontalCentered: true,
                             margins: {left: 0.25, right: 0.25, top: 0.75, bottom: 0.25, header: 0, footer: 0},};
      const pdfBuffer = await workbookToPdfBuffer(workbook, "mapa");
      res.setHeader("Content-Type", "application/pdf");
      return res.status(200).send(pdfBuffer);
    }
    async function handleVacationAnomalies(req, res) {
      ...
      for (let r = ROW_START + rows.length; r <= 110; r++) ws.getRow(r).hidden = true;
      ws.pageSetup = {orientation: "portrait", paperSize: 9, fitToPage: true, fitToWidth: 1, fitToHeight: 0, horizontalCentered: true,
                      margins: {left: 0.3, right: 0.3, top: 0.4, bottom: 0.4, header: 0, footer: 0},};
      const pdfBuffer = await workbookToPdfBuffer(workbook, "discrepancias");
      res.setHeader("Content-Type", "application/pdf");
    ...
    async function handleVacationPriority(req, res) {
      ...
      const ROW_START = 10;
      const ROW_END = 43;
      const safeEmployees = employees.slice(0, ROW_END - ROW_START + 1);
      ...
      safeEmployees.forEach((emp, index) => {
        ...
        for (let i = 0; i < 24; i++) {
          ...
          const cell  = worksheet.getCell(rowNumber, 3 + i);
      ...
      worksheet.pageSetup = {orientation: "landscape", paperSize: 9, fitToPage: true, fitToWidth: 1, fitToHeight: 0, horizontalCentered: true,
                             margins: {left: 0.25, right: 0.25, top: 0.75, bottom: 0.25, header: 0, footer: 0},};
      const pdfBuffer = await workbookToPdfBuffer(workbook, "prioridades");
      res.setHeader("Content-Type", "application/pdf");
      res.setHeader("Content-Disposition", `attachment; filename=Prioridades_${priorityYear}.pdf`);
      return res.status(200).send(pdfBuffer);
    }
    // ─── Salary map shared XML builder ───────────────────────────────────────────
    async function buildSalaryMapXlsx(year, month, employees) {
    ...
    async function handleSalaryMap(req, res) {
      ...
      const {zip, beforeSheetData, afterSheetData, headerRows, dataRowsXml, monthName} = await buildSalaryMapXlsx(year, month, employees);
      ...
      const xlsxBuffer = zip.toBuffer();
      const pdfBuffer = await xlsxBufferToPdfBuffer(xlsxBuffer, `salary_${year}_${month}`);
      res.setHeader("Content-Type", "application/pdf");
    ...
    async function handleSalaryMapXlsx(req, res) {
      const {year, month, employees} = req.body;
      const {zip, beforeSheetData, afterSheetData, headerRows, dataRowsXml} = await buildSalaryMapXlsx(year, month, employees);
      const newSheetData = `<sheetData>${headerRows.join("")}${dataRowsXml}</sheetData>`;
      let newSheetXml = beforeSheetData + newSheetData + afterSheetData;
    ...
    async function handleEIPAnnualMap(req, res) {
      ...
      for (let mi = 0; mi < 12; mi++) {
        ...
        for (let d = 1; d <= MAX_DAYS; d++) {
          ...
          if (d > daysInMonth) {
            [cDay, cWd, cTeam].forEach((c) => {breakStyle(c); c.value = ""; setFill(c, "F8FAFC");});
            continue;
          }
      ...
      const pdfBuffer = await workbookToPdfBuffer(workbook, "eip_annual");
      res.setHeader("Content-Type", "application/pdf");
