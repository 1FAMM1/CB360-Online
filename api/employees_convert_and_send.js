import ExcelJS from "exceljs";
    ...
    import {ServicePrincipalCredentials, PDFServices, MimeType, CreatePDFJob, CreatePDFResult, 
            SDKError, ServiceUsageError, ServiceApiError,} from "@adobe/pdfservices-node-sdk";
    export const config = {
      api: {bodyParser: {sizeLimit: "10mb"}},
    };
    const CLIENT_ID = process.env.ADOBE_CLIENT_ID;
    const CLIENT_SECRET = process.env.ADOBE_CLIENT_SECRET;
    const TEMPLATE_URL = "https://raw.githubusercontent.com/1FAMM1/CB360-Online/main/templates/employees_template.xlsx";
      ...
      try {
        ...
        function setFontKeepTemplate(
          ...
          if (bg === normalizeHex6(DRIVER_BG) || bg === normalizeHex6(FE_BG)) {
            ...
            colorArgb = forceTextColor.toUpperCase().startsWith("FF")
              ? forceTextColor
            : "FF" + forceTextColor;
          } else {
        ...
        worksheet.pageSetup = {orientation: "landscape", paperSize: 9, fitToPage: true, fitToWidth: 1, fitToHeight: 1, horizontalCentered: true, 
                               verticalCentered: false, margins: {left: 0.1, right: 0.1, top: 0.15, bottom: 0.15, header: 0, footer: 0},};
        if (format !== "pdf") {
          ...
          res.setHeader(
            "Content-Disposition",
            `attachment; filename="Escala_Profissionais_${MONTH_NAMES[month - 1]}_${year}.xlsx"`
          );
          return res.status(200).send(Buffer.from(buffer));
        }
        const tempDir = os.tmpdir();
        inputFilePath = path.join(tempDir, `Escala_Profissionais_${month}_${year}_${Date.now()}.xlsx`);
        outputFilePath = path.join(tempDir, `Escala_Profissionais_${month}_${year}_${Date.now()}.pdf`);
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="Escala_Profissionais_${MONTH_NAMES[month - 1]}_${year}.pdf"`
        );
        ...
        return res.status(500).json({
          error: "Erro interno ao gerar/converter",
