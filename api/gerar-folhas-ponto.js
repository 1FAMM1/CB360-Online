...
export default async function handler(req, res) {
  ...
  try {
    ...
    // --- FUNÇÕES DE ESTILO (Originais tuas) ---
    ...
    // --- LÓGICA DE FERIADOS (Ajustada para detectar Carnaval) ---
    function getPortugalHolidays(y) {
      ...
      const carnaval = add(easter, -47);
      const map = new Map();
      
      fixed.forEach(f => map.set(`${y}-${f.m}-${f.d}`, "F7C6C7")); // Rosa
      [add(easter,-2), easter, add(easter,60)].forEach(dt => map.set(`${dt.getFullYear()}-${dt.getMonth()+1}-${dt.getDate()}`, "F7C6C7"));
      map.set(`${carnaval.getFullYear()}-${carnaval.getMonth()+1}-${carnaval.getDate()}`, "D6ECFF"); // Azul Carnaval
      
      return map;
    }

    const holidayMap = getPortugalHolidays(year);
    ...
    for (let d = 1; d <= 31; d++) {
      ...
      const cellEntrada = worksheet.getCell(row, 5); // Adicionado
      const cellSaida = worksheet.getCell(row, 6);   // Adicionado
      ...
      if (d <= daysInMonth) {
        ...
        // 2. Pintar Dia e Semana
        const holidayColor = holidayMap.get(`${year}-${month}-${d}`);
        let dateBg = holidayColor || ((wDay === 0 || wDay === 6) ? WEEKEND_COLOR : null);
        ...
        // 3. Pintar Turno (Com regra de Driver/FE)
        const bgHex = normalizeHex6(employee.cellColors?.[d - 1] || "FFFFFF");
        cellTurno.fill = { type: "pattern", pattern: "solid", fgColor: { argb: "FF" + bgHex } };
        
        // Se for Driver (FF69B4) ou FE (00B0F0), força texto PRETO. Caso contrário, usa luminance.
        let colorArgb = (bgHex === "FF69B4" || bgHex === "00B0F0") ? "FF000000" : (isDarkHex(bgHex) ? "FFFFFFFF" : "FF000000");
        
        cellTurno.font = { bold: true, color: { argb: colorArgb } };
