    ...
    ======================================= */
    /* ─── STYLE CONSTANTS ────────────────────────────────────── */
    const HIGHLIGHT_WORDS = [`Rede Nacional de Áreas Protegidas`, `Rede Natura 2000`, `Linha de Média Tensão`, `áreas de declives acentuados`, `Passagem Superior`,
                             `linhas de média e alta tensão`, `linhas de média tensão`, `atravessa curso de água navegável`, `atravessa curso de água e pontos de água`,
                             `outros compromissos internacionais`, `ALARME ESPECIAL`, `ALARME CONDICIONADO`, 
    ...
    const A22_TITLE_STYLE = `background: linear-gradient(to right,#888,#c0c0c0); color: black; font-weight: bold; font-size: 12px; width: 100%; max-width: 1572px; height: 20px;
    ...
    const INTERVENTION_NOTICE_STYLE = `padding: 10px; margin: 20px 0 5px 0; border-radius: 3px; display: flex; align-items: center; justify-content: center; text-align: center;
    ...
    function playGenericSirene(type, gridId, level, occurrenceType = null) {
      ...
      const mapTypesLinFer  = {"Acidente - Abalroamento, Choque e Descarrilamento" : "Acidente" , "Substâncias Perigosas - Produtos Químicos/Produtos Biológicos" : "SubstanciasPerigosas",
                               "Incêndio em Transportes":"IncendioTransportes"};
    ...
    function renderGenericAlarmCells(tr, alarmArray, i, skipState) {
      ...
      if (corpNr && corp.includes(corpNr)) [td1, td2].forEach(td => {
        td.style.backgroundColor = "#839ec9"; td.style.color = "white"; td.style.fontWeight = "bold";
      });
      tr.append(td1, td2);
    }
    ...
    function createOrUpdateCumulativeAlert(parentContainer) {
      ...
      if (!checkIfHasCBIntervention(parentContainer)) {
        alertDiv?.remove();
        return;
      }
      if (!alertDiv) {
    ...
    function createKmDirectionButtons() {
      ...
      createDirectionButtons(dirRow, [{id:"O-E",line1:"OESTE - ESTE",line2:"Lagos → VRSA"},{id:"E-O",line1:"ESTE - OESTE",line2:"VRSA → Lagos"}],
    ...
    function createPKmDirectionButtons() {
      ...
      createDirectionButtons(dirRow, [{id:"T-L",line1:"TROÇO",line2:"Tunes → Lagos"},{id:"T-V",line1:"TROÇO",line2:"Tunes → VRSA"}],
    ...
    async function loadPPIA22DataSpecials(gridId) {
      ...
      try {
        ...
        if (!p1Div) {
          ...
          Object.assign(p1Div.style,{border:"1px solid #bbb",padding:"0 0 0 8px",marginBottom:"-5px",textAlign:"left"}); p1Div.classList.add("ppia22-part1"); 
          ataContainer.appendChild(p1Div);
        }
    ...
    function renderAeroCustomButtons() {
      ...
      const labelDiv=document.createElement("div"); labelDiv.textContent="GRELHAS DE ALARME";
    ...
    async function loadPPIAeroGridB1(gridId) {
      ...
      try {
        ...
        const titleDiv=document.createElement("div"); titleDiv.textContent="ALERTA VERMELHO";
    ...
    async function loadPPILinFerDataFromKm(km, sentido) {
      try {
        ...
        if(limiteGrelhas.length>1){
          ...
          showPopupWarning(`⚠️ O Ponto Quilométrico <b style="color:red">${km.toFixed(3)}</b> corresponde ao limite entre duas grelhas <b style="color:red">${grelhas.length===2?grelhas.join(" e "):grelhas.join(", ")}</b>.<br>Para garantir maior precisão na atribuição de meios, recomenda-se a consulta direta por grelha. Obrigado.`);
          return;
        }
    ...
    async function loadPPILinFerDataSpecials(gridId) {
      ...
      if(!container.querySelector(".ppilinfer-title")){
        const titleDiv=document.createElement("div"); titleDiv.textContent="SITUAÇÕES ESPECIAIS"; titleDiv.className="ppilinfer-title"; 
        titleDiv.style.cssText=LINFER_SPECIAL_TITLE_STYLE; container.appendChild(titleDiv);
      }
      ...
      try {
        ...
        dataContainer.innerHTML=specials.length?specials.map(s=>{
          const div=document.createElement("div");div.style.cssText=LINFER_SPECIAL_ITEM_STYLE;
          div.innerHTML=formatSpecialsContentWithHighlights(s.content);return div.outerHTML;}).join(""):"Sem informações especiais para esta grelha.";
      } catch(err){
