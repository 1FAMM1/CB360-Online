...
export default async function handler(req, res) {
  ...
  try {
    ...
    // --- LÓGICA DE FERIADOS (Adicionado Carnaval) ---
    function getPortugalHolidays(y) {
      ...
      const holidayData = {};
      fixed.forEach(f => holidayData[`${y}-${f.m}-${f.d}`] = "F7C6C7");
      [add(easter,-2), easter, add(easter,60)].forEach(dt => holidayData[`${dt.getFullYear()}-${dt.getMonth()+1}-${dt.getDate()}`] = "F7C6C7");
      
      const carv = add(easter, -47);
      holidayData[`${carv.getFullYear()}-${carv.getMonth()+1}-${carv.getDate()}`] = "D6ECFF"; // Azul Carnaval
    ...
    for (let d = 1; d <= 31; d++) {
      ...
      if (d <= daysInMonth) {
        ...
        const hColor = holidaysMap[`${year}-${month}-${d}`];
        let dateBg = hColor || ((wDay === 0 || wDay === 6) ? WEEKEND_COLOR : null);
        ...
        // Regra específica: Texto PRETO se for Rosa Driver (FF69B4) ou Azul FE (00B0F0)
        let textColor = (bgHex === "FF69B4" || bgHex === "00B0F0") ? "FF000000" : (isDarkHex(bgHex) ? "FFFFFFFF" : "FF000000");
