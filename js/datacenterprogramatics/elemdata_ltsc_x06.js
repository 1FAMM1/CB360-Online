...
                   children: [{label: "CREPC Algarve"}, {label: "Planeamento Diário"}, {label: "Sitop de Veículos"}, {label: "Refeições DECIR"}]},
                  {label: "Consola de Alarmes"}]},
      {label: "WSMS 360",
       children: [{label: "Ocorrências em Curso"}, {label: "Inserir/Alterar Ocorrência"}, {label: "Encerrar Ocorrência"}, {label: "Solicitar Disponibilidades"}, {label: "Indisponibilidade Veículos"}, {label: "Info. Grelha Município"}, {label: "Serviços EMS"}, {label: "Avisos METEO"}]},
      {label: "Utilitários"},
      {label: "Data Center"}];
    /* ================= LOAD TABLE ================= */
...
      ["win_n_int","win_n_file","win_patent","win_patent_abv","win_abv_name","win_full_name","win_MP","win_TAS","win_section"].forEach(id => {
        const el = document.getElementById(id);
...
          throw new Error(`Erro ao verificar user: ${checkUserGlobal.status}`);
...
          );
          if (!updateUser.ok)
            throw new Error(`Erro ao atualizar user: ${updateUser.status}`);
...
          );
          if (!createUser.ok)
            throw new Error(`Erro ao criar user: ${createUser.status}`);
