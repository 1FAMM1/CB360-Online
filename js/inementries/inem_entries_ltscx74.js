...
async function inemViewPdf(nrCodu, serviceType) {
  ...
  if (ext === "html") {
    fetch(url)
      .then(r => r.text())
      .then(html => {
        const sanitized = html.replace(/window\.print\s*\(\s*\)/g, "void 0");
        iframe.srcdoc = sanitized;
      })
      .catch(() => showPopupWarning("Erro ao carregar o ficheiro HTML."));
  } else {
    iframe.src = url;
  }
...
function createInemEntriesTable() {
  ...
  const COLUMNS = [
    {key: "nr_codu",   label: "Nr. CODU", width: "60px" },
    {key: "alerta",    label: "Alerta",   width: "80px" },
    {key: "vitima",    label: "Vítima",   width: "100px"},
    {key: "morada",    label: "Morada",   width: "200px"},
    {key: "tas",       label: "TAS",      width: "120px"},
    {key: "via",       label: "Via",      width: "60px" },
    {key: "actions",   label: "Verbete",  width: "90px" },
