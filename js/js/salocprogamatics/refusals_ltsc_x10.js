    ...
    async function loadServiceRefusals() {
      try {
        const res = await fetch(`${SUPABASE_URL}/rest/v1/service_refusals?order=refusal_date.desc`, {
          headers: getSupabaseHeaders()
        });
        ...
        if (data.length === 0) {
          ...
          tr.innerHTML = `
            <td colspan="9" style="text-align: center; padding: 20px; color: #666;">
              Não foram encontradas recusas de serviços.
            </td>
          `;
        ...
        data.forEach(item => {
          ...
          tr.innerHTML = `
            ...
            <td style="width: 10px;"><button onclick="deleteServiceRefusal('${item.id}')">🗑️</button></td>
          `;
          tbody.appendChild(tr);
        });
      } catch (error) {
    ...
    async function loadIneInops(yearFilter = null) {
      try {
        let url = `${SUPABASE_URL}/rest/v1/inem_inop?order=ineinop_date.desc`;
        ...
        if (data.length === 0) {
          ...
          tr.innerHTML = `
            <td colspan="8" style="text-align: center; padding: 20px; color: #666;">
              Não foram encontradas inoperacionalidades INEM.
            </td>
          `;
          tbody.appendChild(tr);
          return;
        ...
        data.forEach(item => {
          ...
          tr.innerHTML = `
            ...
            <td>${item.optel_refusal}</td>
            <td>${item.validated_refusal}</td>
            <td style="width: 10px;"><button onclick="deleteIneInop('${item.id}')">🗑️</button></td>
          `;
          tbody.appendChild(tr);
        });
      } catch (error) {
    ...
    async function loadSummaryData() {
      try {
        const inemRes = await fetch(`${SUPABASE_URL}/rest/v1/inem_inop?select=ineinop_shift,ineinop_hour_qtd`, {
          headers: getSupabaseHeaders()
        });
        ...
        const totalD = sumTimes(dRecords);
        const totalN = sumTimes(nRecords);
        const totalAll = sumTimes([...dRecords, ...nRecords]);
        document.getElementById("sum-inop-d").textContent = totalD;
        document.getElementById("sum-inop-n").textContent = totalN;
        document.getElementById("sum-inop-total").textContent = totalAll;
        const refusalsRes = await fetch(`${SUPABASE_URL}/rest/v1/service_refusals?select=id`, {
          headers: getSupabaseHeaders()
        });
        if (!refusalsRes.ok) throw new Error("Falha ao carregar service_refusals");
    ...
    async function loadServiceRefusalsCharts() {
      try {
        const res = await fetch(`${SUPABASE_URL}/rest/v1/service_refusals?select=refusal_date,service_type`, {
          headers: getSupabaseHeaders()
        });
        if (!res.ok) throw new Error(await res.text());
        ...
        if (charts['chart-refusals-type']) {
          const chart = charts['chart-refusals-type'];
    ...
    async function loadIneInopsCharts() {
      try {
        const res = await fetch(`${SUPABASE_URL}/rest/v1/inem_inop?select=ineinop_date,ineinop_shift`, {
          headers: getSupabaseHeaders()
        });
        if (!res.ok) throw new Error(await res.text());
        const registros = await res.json();
        ...
        const totalCounts = {};
        for (let i = 0; i < 12; i++) totalCounts[i] = (dCounts[i] || 0) + (nCounts[i] || 0);
        ...
        createOrUpdateChart('chart-ine-total', monthLabels, monthLabels.map((_, i) => totalCounts[i] || 0), 'Inoperacionalidades Total', 'rgba(54,162,235,0.6)');
      } catch (e) {
        console.error("Erro INEM:", e);
      }
    }
    /* ================ LOAD DASHBORAD ==================== */
