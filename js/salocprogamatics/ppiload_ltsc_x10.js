    /* =======================================
   PPI MODULE — REFACTORED
   ======================================= */

/* ─── STYLE CONSTANTS ────────────────────────────────────── */
const HIGHLIGHT_WORDS = [
  `Rede Nacional de Áreas Protegidas`,`Rede Natura 2000`,`Linha de Média Tensão`,`áreas de declives acentuados`,
  `Passagem Superior`,`linhas de média e alta tensão`,`linhas de média tensão`,`atravessa curso de água navegável`,
  `atravessa curso de água e pontos de água`,`outros compromissos internacionais`,`ALARME ESPECIAL`,`ALARME CONDICIONADO`,
...
const A22_TITLE_STYLE           = `background:linear-gradient(to right,#888,#c0c0c0);color:black;font-weight:bold;font-size:12px;width:100%;max-width:1572px;height:20px;display:flex;justify-content:center;align-items:center;margin:0;`;
...
const AERO_LABEL_STYLE          = {background:"linear-gradient(to right,#888,#c0c0c0)",borderRadius:"3px",height:"30px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"17px",fontWeight:"bold",color:"black",width:"100%",maxWidth:"920px",margin:"3px auto 0 auto"};
...
const LINFER_TITLE_STYLE        = `background:linear-gradient(to right,#888,#c0c0c0);color:black;font-weight:bold;font-size:17px;width:100%;max-width:1580px;height:30px;display:flex;justify-content:center;align-items:center;margin:0 0 5px 0;border-radius:3px;`;
...
function playGenericSirene(type, gridId, level, occurrenceType = null) {
  ...
  switch (type) {
    case 'A22':       soundFolder='A22';       fileName=`${gridId}-${mapTypesA22[occurrenceType]}-${mapAlarmsA22[level]}.mp3`; break;
    case 'Aeroporto': soundFolder='Aeroporto'; fileName=`${gridId}-${level}.mp3`; break;
    case 'LinhaFerrea': soundFolder='LinhaFerrea'; fileName=`${gridId}-${mapTypesLinFer[occurrenceType]}-${mapAlarmsA22[level]}.mp3`; break;
    default: console.error(`Tipo de PPI não reconhecido: ${type}`); return;
  }
...
function createKmDirectionButtons() {
  ...
  createDirectionButtons(dirRow,
    [{id:"O-E",line1:"OESTE - ESTE",line2:"Lagos → VRSA"},{id:"E-O",line1:"ESTE - OESTE",line2:"VRSA → Lagos"}],
...
function createPKmDirectionButtons() {
  ...
  createDirectionButtons(dirRow,
    [{id:"T-L",line1:"TROÇO",line2:"Tunes → Lagos"},{id:"T-V",line1:"TROÇO",line2:"Tunes → VRSA"}],
...
async function loadPPIA22DataFromKm(km, sentido) {
  try {
    ...
    if (!filtered.length) { showPopupWarning("⚠️ Nenhuma grelha encontrada para este quilómetro."); return; }
    ...
    if (limiteGrelhas.length > 1) {
      ...
      showPopupWarning(`⚠️ O quilómetro <b style="color:red">${km.toFixed(2)}</b> corresponde ao limite entre duas grelhas, <b style="color:red">${grelhas.length===2?grelhas.join(" e "):grelhas.join(", ")}</b>.<br>Para garantir maior precisão na atribuição de meios, recomenda-se a consulta direta por grelha. Obrigado.`); return;
    }
  ...
  } catch (err) { console.error("Erro ao carregar dados do Supabase:", err); alert("❌ Erro ao carregar dados. Veja o console."); }
}
...
async function loadPPIA22DataInfoGrid(gridId) {
  ...
  } catch (err) { console.error("❌ Erro ao carregar PPI data:", err); showError(container,"Erro ao carregar PPI data. Veja o console."); }
...
async function loadPPIA22DataSpecials(gridId) {
  ...
  try {
    ...
    if (!p1Div) { p1Div=document.createElement("div"); Object.assign(p1Div.style,{border:"1px solid #bbb",padding:"0 0 0 8px",marginBottom:"-5px",textAlign:"left"}); p1Div.classList.add("ppia22-part1"); dataContainer.appendChild(p1Div); }
    ...
    if (!table) { table=document.createElement("table"); table.classList.add("table-elements"); Object.assign(table.style,{width:"100%",borderCollapse:"collapse",margin:"4px 0 5px 0"}); dataContainer.appendChild(table); }
  ...
  } catch (err) { console.error("❌ Erro ao carregar specials PPIA22:", err); dataContainer.innerHTML="Erro ao carregar informações especiais. Veja o console."; }
}
...
async function loadPPIA22GridSeparated(gridId) {
  ...
  } catch(err) { console.error("❌ Erro ao carregar grelha PPI A22:", err); container.textContent="Erro ao carregar grelha. Veja o console."; }
}
...
function ppiaeroHeaderTable(gridId, label) {
  ...
  if (!table) { table=document.createElement("table"); table.id="ppiaero-grid-header"; Object.assign(table.style,AERO_HEADER_TABLE_STYLE); container.prepend(table); }
  table.innerHTML="";
...
function ppiaeroInfoTable() {
  ...
  if (!table) {
    ...
    const td2=document.createElement("td"); td2.textContent="Esta grelha aplica-se aos cenários de proteção e socorro. Podendo aplicar-se aos restantes cenários definidos pelo PAE Gago Coutinho - Faro, Algarve, desde que solicitado pelas forças de segurança ao CRERPC.";
...
async function loadPPIAeroGridSeparated(gridId) {
  ...
  try {
    ...
    const table=document.createElement("table"); table.classList.add("table-elements"); Object.assign(table.style,{tableLayout:"fixed",width:"100%",margin:"10px 0 5px 0"});
  ...
  } catch(err){console.error("❌ Erro ao carregar grelha PPIAero:",err);container.textContent="Erro ao carregar grelha. Veja o console.";}
}
...
async function loadPPIAeroGridB1(gridId) {
  ...
  try {
    ...
    const wrapper=document.createElement("div"); Object.assign(wrapper.style,AERO_B1_WRAPPER_STYLE);
  ...
  } catch(err){console.error("❌ Erro ao carregar grelha B1:",err);container.textContent="Erro ao carregar grelha. Veja o console.";}
}
...
async function loadPPILinFerDataFromKm(km, sentido) {
  try {
    ...
    if(!filtered.length){showPopupWarning("⚠️ Nenhuma grelha encontrada para este ponto quilométrico no troço selecionado.");return;}
    const limiteGrelhas=filtered.filter(ppi=>{
      const f=parseFloat(ppi.ppi_first_pkm?.match(/PK\s*([\d,]+)/i)?.[1]?.replace(',','.')||0);
      const s=parseFloat(ppi.ppi_secound_pkm?.match(/PK\s*([\d,]+)/i)?.[1]?.replace(',','.')||0);
      return km===f||km===s;
    ...
    if(limiteGrelhas.length>1){
      ...
      showPopupWarning(`⚠️ O Ponto Quilométrico <b style="color:red">${km.toFixed(3)}</b> corresponde ao limite entre duas grelhas <b style="color:red">${grelhas.length===2?grelhas.join(" e "):grelhas.join(", ")}</b>.<br>Para garantir maior precisão na atribuição de meios, recomenda-se a consulta direta por grelha. Obrigado.`);return;
    }
  ...
  } catch(err){console.error("Erro ao carregar dados:",err);alert("❌ Erro ao carregar dados. Veja o console.");}
}
...
async function loadPPILinFerGridSeparated(gridId) {
  ...
  try {
    ...
    container.innerHTML=""; container.appendChild(tempContainer); tempContainer.style.display="block";
    createOrUpdateCumulativeAlert(tempContainer);
  } catch(err){console.error("❌ Erro ao carregar grelha Linha Férrea:",err);container.textContent="Erro ao carregar grelha. Veja o console.";}
}
...
function ppiLinFerABSNoteTable(container, buttonId) {
  ...
  const td=Object.assign(document.createElement("td"),{innerHTML:`<b>* As ABSC</b> a despacho para o PPI da Linha Férrea do Algarve, não devem ser as afetas ao PEM do INEM, sendo complementares às mobilizadas pelo CODU do INEM (GRELHA ${letter} - INEM)`});
