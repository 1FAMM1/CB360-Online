...
function buildEmailTemplate({title, subtitle, logoUrl, emailBody, corpName, corpAddress, corpCp, corpLocalitie, corpPhoneMobile, corpPhoneLandline}) {
  const addressLine = [corpAddress, [corpCp, corpLocalitie].filter(Boolean).join(" ")].filter(Boolean).join(" | ");
  const phoneLine = [
...
async function handleMOA(req, res) {
  ...
  const htmlEmail = buildEmailTemplate({
    title: corpName,
    subtitle: "Medidas Operacionais de Antecipação",
    logoUrl: data.logoUrl || "",
    emailBody: emailBody || `<p>Segue em anexo a MOA.</p>`,
    corpName,
    corpAddress: data.corpAddress || "",
    corpCp: data.corpCp || "",
    corpLocalitie: data.corpLocalitie || "",
    corpPhoneMobile: data.corpPhoneMobile || "",
    corpPhoneLandline: data.corpPhoneLandline || "",
  });
...
async function handleSITOP(req, res) {
  ...
  const htmlEmail = buildEmailTemplate({
    title: corpName,
    subtitle: `Situação Operacional de Veiculos - ${prefix} ${data.vehicle}`,
    logoUrl: data.logoUrl || "",
    emailBody: emailBody || `<p>Segue em anexo o documento de Situação operacional do veiculo ${data.vehicle || ""}.</p>`,
    corpName,
    corpAddress: data.corpAddress || "",
    corpCp: data.corpCp || "",
    corpLocalitie: data.corpLocalitie || "",
